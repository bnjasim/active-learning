...
out_prob = tf.nn.softmax(y_conv)
pos99 = (tf.reduce_max(out_prob, axis=1) >= 0.99)
labels99 = tf.one_hot(tf.argmax(tf.boolean_mask(out_prob, pos99), axis=1), 10)

def unsuper_fn(pool_data, step=None):
    # probs = sess.run(out_prob, feed_dict={x: pool_data, keep_prob1:1.0, keep_prob2:1.0})
    # pos = (np.max(probs, axis=1) >= 0.99)
    # labels = np.argmax(probs[pos], axis=1)
    pos, labels = sess.run([pos99, labels99], feed_dict={x: pool_data, keep_prob1:1.0, keep_prob2:1.0})
    return pos, labels
...
var_acc = a.run(50, [random_acq, var_ratio_tf], unsuper_fn, pool_subset_count=1000, unsup_pool_size=1000)
